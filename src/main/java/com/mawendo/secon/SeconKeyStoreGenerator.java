...
public class SeconKeyStoreGenerator {
  ...
  public static void main(String[] args) {
    ...
    options.addOption(
        new Option(
            "k", "insurance-keys", true, "path of the insurance keys file (default: " + DEFAULT_KEY_PATH + ")"));
    ...
    options.addOption(
      ...
      options.addOption(
              new Option(
                      ...
                      "private-key",
                      true,
                      "(optional) path of your private key (PKCS1 .pem file beginning with '--BEGIN RSA PRIVATE KEY--')"));
      ...
      options.addOption(new Option("a", "alias", true, "the alias of your private key in the keystore (default: " + DEFAULT_PRIVATE_KEY_ALIAS + ")"));
    ...
    if (cmd.hasOption("private-key") || cmd.hasOption("chain")) {
        if (!cmd.hasOption("chain") || !cmd.hasOption("private-key")) {
            Logger.error(
                    "To embed your private key in the keystore you must provide both a PEM private key (--private-key) and a P7C certificate chain (--chain)"
            );
            formatter.printHelp(SeconKeyStoreGenerator.class.getSimpleName(), options);
            System.exit(1);
        }
    ...
    Path keyFilePath =
        Paths.get(cmd.hasOption("insurance-keys") ? cmd.getOptionValue("insurance-keys") : DEFAULT_KEY_PATH);
